...
export default function playerDashDrawer({canvas, currPosition, dashNumber, angle, lastPosition, direction, filter}) {
    ...
    if (dashNumber < 4 && !filter) {
        get_image("dash/animation", "dash_animation", dashNumber, (img) => {
    ...
    if (direction === "w") {
        return get_image("dash", "dash_up", dashNumber, function (img) {
            canvas.drawImage(img, currPosition.x - 25, currPosition.y - 10, img.width * 2, img.height * 2);
        });
    }
    if (direction === "d") {
        return get_image("dash", "dash_side", dashNumber, function (img) {
            canvas.drawImage(img, currPosition.x - 25, currPosition.y - 10, img.width * 2, img.height * 2);
        });
    }
    if (direction === "a") {
    ...
    if (direction === "s") {
        return get_image("dash", "dash_down", dashNumber, function (img) {
            canvas.drawImage(img, currPosition.x - 25, currPosition.y - 10, img.width * 2, img.height * 2);
        });
    }
}
